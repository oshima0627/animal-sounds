...
|:---:|:---:|
| アプリ起動時にロゴがバウンスして表示 | 2列グリッドで14匹の動物を表示 |
...
| `sound_horse.mp3` | 馬（ヒヒーン！） |
| `sound_cow.mp3` | 牛（モーモー！） |
| `sound_chicken.mp3` | ニワトリ（コケコッコ！） |
| `sound_chick.mp3` | ヒヨコ（ピヨピヨ！） |
| `sound_sheep.mp3` | ヒツジ（メェーメェー！） |
| `sound_goat.mp3` | ヤギ（メーメー！） |
| `sound_lion.mp3` | ライオン（ガオー！） |
| `sound_elephant.mp3` | ゾウ（パオーン！） |
| `sound_wolf.mp3` | オオカミ（アオーン！） |
| `sound_boar.mp3` | イノシシ（ブヒブヒ！） |
| `sound_crow.mp3` | カラス（カーカー！） |
| `sound_sparrow.mp3` | スズメ（チュンチュン！） |
...
- [ ] 動物イラストを手書き風の画像に差し替え
- [ ] 動物ごとに異なるアニメーション（ライオンは咆哮、馬は駆け足等）
- [ ] タブレット向け 3列グリッド対応
- [ ] 動物の数をさらに増やす（ペンギン、パンダ等）
- [ ] 鳴き声の種類を複数登録してランダム再生
- [ ] 子供の操作ログ（親向け）
